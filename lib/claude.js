...
export async function generatePredictionExplanation(input) {
  ...
  const prompt = `You are a professional football analyst explaining MatchMind's prediction.

The prediction was generated by a Dixon-Coles Poisson model with home advantage (1.25x), Elo ratings, weighted form, rest days, and H2H adjustments. Your job is to explain WHY the model made this prediction using the data below.
...
Rules:
- Be specific — reference actual numbers from the features (form points, Elo, xG, clean sheet rates)
- Never say "the model predicts" — say what the DATA shows
- Key stat should be the single most compelling datapoint
- Watch out should be a genuine risk factor that could flip the result
- Each reason should be max 15 words and data-backed`
  ...
  try {
    ...
    return { summary: text, reasons: [], key_stat: '', watch_out: '' }
  }
}
...
export async function generateAccumulatorExplanation(input) {
  ...
  const response = await client.messages.create({
    model: 'claude-sonnet-4-20250514',
    max_tokens: 1000,
    system: `You are MatchMind's accumulator analyst. Given a list of match predictions, return a JSON object explaining the accumulator.
You must ONLY respond with a valid JSON object — no text before or after. No markdown code blocks. Pure JSON only.
Return JSON only in this format:
{
  "summary": "<one sentence overall take on this accumulator>",
  "reasoning": "<2-3 sentences explaining why these selections work together>",
  "risk_level": "Low | Medium | High"
}`,
    messages: [{ role: 'user', content: `Accumulator selections:\n${JSON.stringify(selections, null, 2)}` }]
  })
  ...
  try {
    ...
    console.error('[Claude] Failed to parse accumulator response:', text.slice(0, 300))
    throw new Error('AI returned invalid accumulator format')
  }
}
