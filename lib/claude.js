...
export async function generatePredictionExplanation(matchData, prediction) {
  const response = await client.messages.create({
    model: 'claude-sonnet-4-20250514',
    max_tokens: 500,
    system: `You are MatchMind's AI analyst. Write a short, clear explanation of a football prediction in 2-3 sentences. Be specific and data-driven. Plain text only — no JSON, no markdown.`,
    messages: [{
      role: 'user',
      content: `Match: ${matchData.home_team} vs ${matchData.away_team} (${matchData.league})
Prediction: ${prediction.outcome} with ${prediction.confidence}% confidence
Key reasons: ${Array.isArray(prediction.reasons) ? prediction.reasons.join('; ') : prediction.reasons}
Write a concise explanation for this prediction.`
    }]
  })
...
export async function generateAccumulatorExplanation(selections) {
  const selectionsList = selections.map((s, i) =>
    `${i + 1}. ${s.home_team} vs ${s.away_team}: ${s.outcome} (${s.confidence}% confidence)`
  ).join('\n')

  const response = await client.messages.create({
    model: 'claude-sonnet-4-20250514',
    max_tokens: 600,
    system: `You are MatchMind's AI analyst. Write a short summary for an accumulator bet in 3-4 sentences. Highlight the strongest picks and any risks. Plain text only — no JSON, no markdown.`,
    messages: [{
      role: 'user',
      content: `Accumulator selections:\n${selectionsList}\nWrite a concise summary for this accumulator.`
    }]
  })
